...
1. Populate the `spec.cluster.workloadArchitectures` field in the `SSP` CR with the architectures of the workload nodes
   currently present in the cluster.

2. Populate the `spec.cluster.controlPlaneArchitectures` field in the `SSP` CR with the architectures of the
...
`DataSource` CRs to be the default one. If there is only one supported architecture, this will be the default one.
