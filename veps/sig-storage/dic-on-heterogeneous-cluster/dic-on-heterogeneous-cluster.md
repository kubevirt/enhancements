...
1. The new `allowHeterogeneousCluster` feature gate is enabled in the `HyperConverged` CR. This feature gate is disabled 
   by default.
2. The cluster is not a single node cluster.

When reconciling the `SSP` CR, HCO will:

1. Populate the `spec.cluster.workloadArchitectures` filed in the `SSP` CR with the architectures of the workload nodes
   currently present in the cluster.

2. Populate the `spec.cluster.controlPlaneArchitectures` filed in the `SSP` CR with the architectures of the
...
3. The pre-prepared DataImportCronTemplate files in the HCO image, will already contain the 
   `ssp.kubevirt.io/dict.architectures` annotation for each `DataImportCronTemplate`. When adding the a
   `DataImportCronTemplate` object to the `SSP` CR, HCO will clean the architectures that are not present in cluster
    from the annotation.
   
   If there are no architectures that supported by the image and present in the cluster, HCO will not add this
   `DataImportCronTemplate` object to the `SSP` CR.

4. HCO will not add the `ssp.kubevirt.io/dict.architectures` annotation to the custom `DataImportCronTemplate` objects.
...
2. Set the `spec.template.spec.source.registry.platform.architecture` field in the `DataImportCron` CR, to the
   architecture of the image. This field is used by the cdi-importer to import the desired image from the registry.
3. Add the `template.kubevirt.io/architecture` label with the required architecture.
4. Add the `cdi.kubevirt.io/storage.import.datasource-name` label with the value of `DataImportCronTemplate`'s
   `spec.managedDataSource` field.
5. SSP will not copy the `ssp.kubevirt.io/dict.architectures` annotation to the `DataImportCron` CR, to avoid
   confusion.
...
`DataImportCronTemplate` object, SSP will maintain the current behavior by creating a single `DataImportCron`
...
When reconciling the `DataSource` CR, CDI will add the architecture to the `instancetype.kubevirt.io/default-preference`
label, to be in the format of `<os>.<version>.<arch>`, e.g. `centos.stream9.amd64`.
...
To achieve that, CDI API will introduce the new "pointer" DataSource, by adding the `spec.source.dataSource` filed to
the DataSource API. 

SSP will create the architecture agnostic `DataSource` CR with no architecture label, and with the original name, 
as listed in the `spec.managedDataSource` field in the `DataImportCronTemplate` object (without architecture suffix). 
This `DataSource` CR point to the default `DataSource` CR, as described below.

For each image (represented by the same `DataImportCronTemplate` object in the SSP CR), SSP will select one of the
`DataSurce` CRs to be the default one. If there is only one supported architecture, this will be the default one.
If there are more than one supported architecture, the default one will be the one with the same architecture as the
control-plane architecture, listed in the `spec.cluster.controlPlanArchitectures` field in the `SSP` CR.

> **Note**: The `spec.cluster.controlPlanArchitectures` field contains only one architecture, as the control-plane
...
On upgrade from a previous version of HCO, or when the `allowHeterogeneousCluster` feature gate is enabled in the
HyperConverged CR, there will be already an existing `DataSource` CR for the specific image. SSP will modify the
...
`spec.cluster.workloadArchitectures` `SSP` CR's field. SSP dynamically add or remove templates if the architecture list
...
Assuming the user knows the architecture of the VM they want to create, the kuevirt-plugin (UI) will prompt the user to
...
// +optional
//Variant specifies the image target CPU architecture variant
...
type DataSourceSource struct {
	...
	// +optional
	Blank *DataSourceSourceDataSource `json:"dataSource,omitempty"`
}
```

```go
type DataSourceSourceDataSource struct {
...
In addition, the `SSP` CR's `spec` field will be extended with the new `cluster` with a new struct type, similar to:
...
The HyperConverged API will introduce the new `allowHeterogeneousCluster` feature gate, with default value of `false`.
...
1. Add the new `cluster` field to the `SSP` CR.
