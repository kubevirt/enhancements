...
The HCO will be enhanced to track the architectures of workload nodes in the cluster. It will maintain a distinct list
of these architectures, derived from the `status.nodeInfo.architecture` field of each node.

This list will be dynamically updated as nodes are added or removed from the cluster.
...
2. The pre-prepared files will already contain the `ssp.kubevirt.io/dict.architectures` annotation for each
   `DataImportCronTemplate`. When adding the a `DataImportCronTemplate` object to the `SSP` CR, HCO will clean the
   architectures that are not present in cluster from the annotation.
...
3. HCO will not add the `ssp.kubevirt.io/dict.architectures` annotation to the custom `DataImportCronTemplate` objects.
...
both `amd64` and `arm64`, the `SSP` CR will look like this:
  ...
  commonTemplates:
    dataImportCronTemplates:
      - metadata:
          ...
          annotations:
            ssp.kubevirt.io/dict.architectures: "arm64,amd64"
    ...
```
