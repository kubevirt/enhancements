# VEP #NNNN: Declarative Volume Hotplug
...
#### Option 1 - LiveUpdate VM Rollout Strategy + Hotplug Volume Update Strategy
...
#### Option 2 - New default (most of the time)
...
Cons:
- Not backward compatible (can have a feature gate initially)
- Can't hotplug volumes during storage migration unless we don't combine subresource and declarative APIs

This option is rejected becasue of backwards compatibality concerns
...
Should be no issues on upgrade given that users have to opt in by setting `Hotplug` Volume Update Strategy. Rollback should restore previous behavior without issue.
...
`Hotplug` Volume Update Strategy will be feature gated. Users will have to opt in.
