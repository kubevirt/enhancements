# VEP #326: Expose `directsync` disk cache mode
...
The `directsync` cache mode is not a new hypervisor feature. It has been stable, well-understood primitive in QEMU and libvirt for over a decade:
...
- Community users have independently requested `directsync` and `unsafe` modes: [kubevirt#14265](https://github.com/kubevirt/kubevirt/issues/14265), [kubevirt#14363](https://github.com/kubevirt/kubevirt/pull/14363), [harvester#7906](https://github.com/harvester/harvester/issues/7906).

This proposal does not introduce a new API field; it adds one value to an existing `DriverCache` enum that already exposes three of libvirt's five cache modes. The validation, defaulting, and I/O path logic already exist. A feature gate would add process overhead without meaningful risk reduction for values that are over a decade old in the underlying stack.
...
KubeVirt currently exposes three disk cache modes to users: `none`, `writethrough`, and `writeback`. The underlying hypervisor (QEMU/libvirt) supports two additional modes -- `directsync` and `unsafe` -- that address specific performance and data-safety trade-offs not covered by the existing three. This VEP proposes adding the `directsync` one as first-class `DriverCache` value in the KubeVirt API, along with the validation, defaulting, and test coverage required to support it.
...
- Add `directsync` as valid value for `DriverCache`.
...
- Validate that all cache modes, including the new one, function correctly through functional tests covering migration and data integrity.
...
- **VM administrators** who need to tune disk I/O for specific guest workloads (databases).
...
- The webhook validates that the provided cache string is one of the four known values.
...
4. **Expose `unsafe` as well**: This has been rejected during PR and VEP review.
...
- **Forward compatible**: Existing VMIs with `none`, `writethrough`, or `writeback` are unaffected. New VMIs specifying `directsync` are only valid on clusters running the version that introduces these values.
...
- **Validation**: Unit tests cover admission validation for all four cache modes, including shareable disk restrictions and direct I/O fallback behavior.
...
- 2026-09-07: removed `unsafe`

## Graduation Requirements

### GA (v1.11)

- `directsync`  accepted as valid `DriverCache` values.
...
- Functional tests for live migration with the new cache mode.
- Documentation updated in kubevirt.io user guide.
- `virtctl` accepts the new mode in cache-related flags (e.g. hotplug volume).
