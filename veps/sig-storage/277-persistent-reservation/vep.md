...
SCSI Persistent Reservation allows KubeVirt VMs to use SCSI-3 PR commands to coordinate and restrict access to shared block storage.  
This prevents data corruption when multiple VMs or processes access the same LUN, and is commonly used for workloads like Windows Server Failover Clustering (WSFC).
...
Enable persistent reservation on a LUN disk in the VMI spec:
```yaml
devices:
  disks:
  - name: mypvcdisk
...
Enable feature by patching KubeVirt CR:
```yaml
spec:
  configuration:
...
- KubeVirt CR configuration is currently the source of truth. However, for backward compatibility, if the feature gate is defined it will also be interpreted as enabled.
