...
* **Pull mode**: In this mode, an estimation of the backup size is performed beforehand and a filesystem-based PVC is provisioned by the backup controller. The provisioned PVC serves as temporary storage for a scratch file used to facilitate the backup process by storing further writes during it. Once the backup is initialized, libvirt creates a Unix domain socket that exposes the NBD export internally. A user-facing endpoint is then exposed to allow external components to interact with the NBD server, e.g., to retrieve the backup data or query the dirty bitmap of an export.
...
In `push` mode, the backup job in libvirt automatically terminates once all data has been successfully backed up. The controller will be notified when the job completes and will update the `VirtualMachineBackup` phase to `Done`. At this stage, the PVC will be detached from the VM and made available for user operations. Since the `VirtualMachineBackup` resource is no longer needed, it can be safely deletedIn `pull` mode, the backup job does not terminate automatically since there's nothing to indicate completion. Instead, once the backup can be retrieved by a client, the `VirtualMachineBackupStatus` will be changed to `Ready` field and a `Ready` condition. At that point, it is the client's responsibility to explicitly delete the associated `VirtualMachineBackup` resource to signal that the backup process has concluded. This deletion triggers the cleanup of all related artifacts, including aborting the active libvirt backup job, removing the controller-provisioned scratch PVC, and deleting the Unix socket inside the virt-launcher.
In `pull` mode, the backup job does not terminate automatically since there's nothing to indicate completion. Instead, once the backup can be retrieved by a client, the `VirtualMachineBackupStatus` will be updated with a a `Ready` condition. At that point, it is the client's responsibility to explicitly delete the associated `VirtualMachineBackup` resource to signal that the backup process has concluded. This deletion triggers the cleanup of all related artifacts, including aborting the active libvirt backup job, removing the controller-provisioned scratch PVC, and deleting the Unix socket inside the virt-launcher.
...
- `includedVolumes` <br>
A list of the names of the volumes that were included for the backup.
...
**Pull mode:**
Clients connect via HTTP to endpoint(s) exposed in the `VirtualMachineExportStatus` of a backup source `VirtualMachineExport` (see `Pull Mode M.O. and Alternatives` for more information). The connection is secured with TLS and authorized using a user-provided token. The client can query the endpoint(s) in order to either retrieve the dirty bitmap of a backup export or pull ranges of data.
...
sequenceDiagram
    ...
    BC->>Launcher: RPC BackupConnect(IP, CA)
...
Internal traffic and external traffic are both secured using the existing export certificate API.
...
Q: How is the scratch PVC size determined?

A: A basic heuristic of a sum of 10% of each included volume will be used to calculate the size. Further extensions of this VEP may introduce a user-configurable field that controls the percentage.
