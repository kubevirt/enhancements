...
|                         |     |  - patch spec           |     |  - wait libvirt detach  |
...
Delivery is the only phase that can run either off-node (controllers, attachment Pods, CSI provisioners) or on-node (DaemonSets, sidecars), and it is intentionally pluggable. Its only contract with the rest of the pipeline is a single output consumed by Phase 2: an **absolute host path** where the backing storage is visible on the target node. Phase 2 `stat`s the path to determine whether it is a block device or a regular file, and picks the corresponding staging step (`mknod` or bind-mount) accordingly.
...
type NodeLocalDeviceSource struct {
    Path   string                `json:"path"`                 // required: host path
...
message Device {
  ...
  TargetBus target_bus = 2;  // TARGET_BUS_VIRTIO (default) | TARGET_BUS_SCSI | TARGET_BUS_SATA
  string serial = 3;         // optional
}
...
3. **Node-local hotplug service:**
   1. **Validates** — VMI is `Running`, not being deleted, not migrating; `device_path` is absolute; it exists and `stat` reports either a block device or a regular file; `target_bus` is valid.
   2. **Idempotency check** — if a volume with the same `volume_name` already exists in `spec.volumes` with matching `device_path`, the service short-circuits and returns `success=true, reason="AlreadyStaged"` without re-staging.
...
```mermaid
sequenceDiagram
    client ->> NLH gRPC server: AttachDevice(volName, device_path ...)
    NLH gRPC server ->> NLH gRPC server: Validate (VMI running, device_path, format(stat), bus)
