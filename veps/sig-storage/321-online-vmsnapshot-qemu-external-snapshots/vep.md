...
The new flow is enabled by setting `spec.snapshotMethod: External` on the
...
2. Creates a PVC (`{vmsnapshot-name}-snap-scratch`) sized to hold qcow2 overlay files
...
calls a new asynchronous `CommitSnapshot` subresource on the VMI. Inside
...
and the VMSnapshotContent's `status.snapshotMethod`. If the method is
...
2. The VM controller's `startVMI()` checks `vm.status.snapshotInProgress`,
   ...
   and checks `content.status.snapshotMethod`. If not `"External"`,
   no recovery is needed.
3. The VM controller derives the scratch PVC name
   (`{vmsnapshot-name}-snap-scratch`), verifies it exists, and patches it
   as a utility volume onto the new VMI. The VMI create admitter rejects
...
sequenceDiagram
    ...
    VMC->>VMC: GET VMSnapshot → Content → snapshotMethod == External
...
spec:
  source:
    ...
    kind: VirtualMachine
    name: my-windows-vm
  snapshotMethod: External        # optional, defaults to Direct (current flow)
...
status:
  snapshotMethod: External          # new field, set at creation
  readyToUse: false
  volumeSnapshotStatus:
  - volumeSnapshotName: vmsnapshot-my-snapshot-disk-0
```

The `snapshotMethod` field indicates how the snapshot was taken. `"External"`
...
The scratch PVC name is derived from the VMSnapshot name
(`{vmsnapshot-name}-snap-scratch`), which allows the VM controller to find
it during recovery.
