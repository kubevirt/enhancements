...
> [!NOTE]
> The virt-handler and virt-launcher are unaffected, as they identify the network naming scheme by reading info directly
...
A network domain mutator will be registered to the hook mechanism.
The network domain mutator will scan for interfaces connected to secondary networks that utilize tap-based bindings.
...
The network domain mutator serves as a temporary upgrade mechanism. It is scheduled for removal after three versions,
concurrently with the remaining logic for the ordinal naming scheme.
