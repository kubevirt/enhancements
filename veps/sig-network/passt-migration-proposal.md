...
To maintain storage efficiency and eliminate unnecessary security risks, the `passt-helper` binary will be extracted from the `passt` RPM and installed in the virt-handler container image without the `passt` binary.
...
Once the cluster is fully upgraded to the new KubeVirt release, VMs running older versions of `passt` network binding, should be restarted.
...
This ensures that there's no chance of an old virt-launcher starting-up with a new `passt` sidecar version, as this combination will not work as described [here](###Interoperability).
Once the entire system and config is updated, `passt` based VMs should be restarted to benefit from new `passt` features. If they're not restarted, they are expected to continue to work as before.
...
### Interoperability
The solution consists of the following versioned execution units:
- `passt` is released as part of the virt-launcher container image. It is also tied to a libvirt and qemu release.
- `passt-repair` and its calling logic is released as part of the virt-handler container image.
- `passt` binding plugin sidecar (reminder: constructs DomainXML net interface) is a standalone container image, and its version tag is configured in the cluster wide KubeVirt configuration.
- `passt` CNI plugin is a standalone binary built as part of the KubeVirt release. It is the only part of the solution that is not undergoing any change, therefore omitted from the interoperability matrix.

| old version  /  new version | `passt/virt/qemu`                                            | `passt-repair`                        | Sidecar                                |
|-----------------------------|--------------------------------------------------------------|---------------------------------------|----------------------------------------|
| `passt/virt/qemu`           |                                                              | Migration works but connection breaks | <font color="red">Will not work</font> |
| `passt-repair/virt-handler` | Migration works but connection breaks                        |                                       | No interaction                         |
| Sidecar                     | Like old `passt`: no vhost-user, migration breaks connection | No interaction                        |                                        |

In summary, the only combination that will not work is a new sidecar with an old virt-launcher image, because it will format a DomainXML with vhost-user, which is not yet supported by libvirt and passt.
Other interoperability combinations degrade functionality to its current capabilities. 
