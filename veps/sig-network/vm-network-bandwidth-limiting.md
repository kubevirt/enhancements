...
```mermaid
flowchart TD
    U([User / Instancetype]) --> |"1. Apply VM Spec (limit: 100M)"| API[KubeVirt API Webhook]
    
    API --> |"2. Validate & Reject if Primary<br>3. Accept VM Object"| VC[virt-controller]
    
    VC --> |"4. Read Base NAD & Extract Bandwidth<br>5. Inject Multus JSON Annotation"| VL[virt-launcher Pod]
    
    %% Visual representation of the injected JSON
    JSON_NOTE["k8s.v1.cni.cncf.io/networks: <br> [{'name': 'base-nad', 'bandwidth': {'ingressRate': '100M'}}]"] -.-> VL
    style JSON_NOTE fill:#f9f2f4,stroke:#d04437,stroke-width:1px,stroke-dasharray: 5 5
    
    VL --> |"6. Pod Boot Sequence"| M((Multus CNI))
    
    M --> |"7. Apply Base Network"| B[Base CNI]
    B --> |"8. Enforce QoS"| BW[Bandwidth CNI Plugin]
```
