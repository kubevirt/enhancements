...
* **Datapath selection:** Allow admins to choose between direct virt-handler connectivity (`Direct`) and sync-controller proxying (`Proxy`) via `decentralizedLiveMigrationDatapath`. All clusters that participate in cross-cluster decentralized live migration with each other must use the same datapath setting; a mix of `Direct` and `Proxy` across clusters is unsupported and will cause migrations to fail.
...
* Mixed `Direct` and `Proxy` datapaths across clusters in the same cross-cluster migration federation.
...
### Synchronization controller leader election

The synchronization controller deployment runs multiple replicas (two by default) for high availability. Only one replica is active at a time; the others are idle standbys.

Leader election uses a Kubernetes `Lease` named `virt-synchronization-controller`. The elected leader is the only replica that:

* runs the reconciliation loop,
* listens for incoming peer gRPC connections, and
* proxies migration data when `decentralizedLiveMigrationDatapath` is `Proxy`.

Standby replicas do not accept synchronization or migration-proxy traffic. This prevents cross-cluster peers from connecting to an idle replica.

`status.synchronizationAddresses` is derived from the lease holder's pod (the active leader), not from an arbitrary replica. When leadership fails over, the new leader acquires the lease, starts serving, and the advertised address is updated accordingly.
...
**Cross-cluster datapath consistency**

`decentralizedLiveMigrationDatapath` is configured per cluster. For cross-cluster decentralized live migration, every cluster in the migration path must use the same setting. If the source cluster uses `Proxy` and the target uses `Direct` (or vice versa), the two sides expect incompatible migration-data paths and the migration will not succeed.

KubeVirt does not coordinate or validate datapath settings across clusters; ensuring a consistent configuration is the responsibility of the cluster admin or migration orchestrator. Within a single cluster, cross-namespace decentralized migrations always use that cluster's datapath setting on both sides.
...
For the `network` set / `crossClusterNetwork` unset row, `status.synchronizationAddresses` reflects the pod-network peer endpoint, not `migration0`.
...
Configurations without `crossClusterNetwork` route peer proxy traffic over the pod network, exposed through Service/Ingress/Route. `status.synchronizationAddresses` advertises the pod-network address of the active leader when no Ingress or Route is configured, or the externally reachable Ingress/Route URL when one is present. This is always the peer endpoint—not the `migration0` address—even when `migrations.network` is set. Orchestrators use this value as the target `connectURL` on the source `VirtualMachineInstanceMigration` (see [VEP #24](../24-decentralized-migration/vep.md)).
...
source sync controller ──(pod network / ingress/route)──► target sync controller
target sync controller ──(migration0)──► target virt-handler
```
...
source sync controller ──(ingress/route)──► target sync controller
...
**Note:** Service/Ingress exposure for peer proxy traffic still requires a sig-network review. Details will be captured in a follow-up VEP update; this is not a blocker for the proxy datapath work described here.
...
The `connectURL` is the Ingress/Route hostname and port published in the target cluster's `status.synchronizationAddresses`. It must be reachable from the source cluster and route to the target sync-controller gRPC port.
...
source sync controller ──(crosscluster0)──► target sync controller
target sync controller ──(pod network)──► target virt-handler
```
