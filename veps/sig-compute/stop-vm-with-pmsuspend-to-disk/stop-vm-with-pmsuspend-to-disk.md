...
A user exec `virtctl stop vm.name -n vm.namespace --suspend-to-disk=true` commend, the vm stopped with memory state saved, exec `virtctl start vm.name` to start the stopped vm.
