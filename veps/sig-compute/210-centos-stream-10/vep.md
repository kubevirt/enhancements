...
Hardware-specific lanes (sriov, vgpu, windows, sev) and other specialized lanes (ipv6, arm64, migrations) that currently run on the latest Kubernetes version will switch to CentOS Stream 10 in Phase 1 alongside the k8s-1.36 core SIG lanes. The exact timing for each specialized lane can be adjusted based on readiness.
