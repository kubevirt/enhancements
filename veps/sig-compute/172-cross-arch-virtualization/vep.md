...
- This VEP targets alpha for version: v1.9.0, 1.10.0
...
- Allow cluster administrators and users to configure a maximum emulation tier
  so that emulation remains explicitly opt-in and workloads
...
cluster-wide `Hardware` policy for that specific workload.
...
- **Downgrading to a previous version**: The `emulationPolicy` field is ignored; existing VMs continue unaffected until they are rescheduled, new VMs cannot use the policy
...
- Live migration of cross-arch VMs is no longer possible
...
- [ ] Basic test coverage for software emulation (E2E tests)
...
- [ ] Decide if `emulationPolicy` should also replace `useEmulation` to consolidate emulation under a single setting
...
- [ ] Clarify live-migration compatibility
