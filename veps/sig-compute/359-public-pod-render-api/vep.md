...
Beyond external consumers, the render module can serve as the foundation for
...
in a specific order, then calls `RenderLaunchManifest`. The render module
encapsulates this entire pipeline behind a single function call with an explicit
`Options` struct. Migrating `virt-controller` to use the render module is a
larger refactor that would follow as a separate effort after the module is
established.
...
benefit future hypervisor abstraction efforts
([VEP-97](https://github.com/kubevirt/enhancements/pull/97)).

## Goals

- Expose a public Go module (`kubevirt.io/render`) for rendering a Pod spec
  from a VirtualMachine or VirtualMachineInstance definition.
...
- **Exposing `ClusterConfig` internals**. The public API accepts feature gates
  ...
  `pkg/virt-config`.
- **Cross-version compatibility**. The rendered Pod spec is specific to the
  KubeVirt version the module was built from. A Pod rendered with one version
  is not expected to work with a different version's virt-launcher.
...
// applying VM defaults. Useful for tools that need the VMI without full Pod
// rendering.
func VMIFromVM(vm *virtv1.VirtualMachine, opts Options) (*virtv1.VirtualMachineInstance, error)
```

All functions are stateless, pure transformations, and safe for concurrent use.
...
type Options struct {
    ...
    // DisabledFeatureGates is the list of feature gates to explicitly disable.
    // Useful for disabling Beta feature gates that are on by default.
...
internal KubeVirt components where possible. `RenderConfig` captures the
configuration values needed by the rendering pipeline (feature gates, machine
types, resource defaults, network settings). `ManifestRenderer` is responsible
for producing the final Pod manifest.

The existing `*virtconfig.ClusterConfig` and `*services.TemplateService`
satisfy these interfaces without adapters, so virt-controller's existing code
path continues to work unchanged. For offline use, the render module provides
its own lightweight `RenderConfig` implementation built from `Options` values
— no informers, goroutines, or Kubernetes client required. The specific
interface methods are defined in the implementation and may evolve as the
module matures.
...
[VEP-97](https://github.com/kubevirt/enhancements/pull/97) (alternative
hypervisors). As KubeVirt
...
Internally, `PodFromVM` and `PodFromVMI` execute the same sequence of VM/VMI
defaults, mutations, network configuration, and manifest rendering that
KubeVirt's admission webhooks and virt-controller perform today. The render
module centralizes this pipeline behind the public API so that:
...
- [ ] `PodFromVM`, `PodFromVMI`, and `VMIFromVM` functions implemented
- [ ] `Options` struct covers launcher image, feature gates (including
  `DisabledFeatureGates`), export image, and run-as user
...
- [ ] `RenderConfig` and `ManifestRenderer` interfaces defined and used by
  the rendering pipeline
- [ ] Downstream packages (`pkg/defaults`, `mutators`, `TemplateService`)
  refactored to accept interfaces instead of concrete `*ClusterConfig`
- [ ] Offline `RenderConfig` implementation provided (built from `Options`,
  no informers or Kubernetes client required)
- [ ] Render package no longer depends on `pkg/virt-config` directly
