# VEP #385: Guest Device Info Metrics

## VEP Status Metadata

### Target releases

- This VEP targets alpha for version: v1.10
- This VEP targets beta for version:
- This VEP targets GA for version:

### Release Signoff Checklist

Items marked with (R) are required *prior to targeting to a milestone / release*.

- [ ] (R) Enhancement issue created, which links to VEP dir in [kubevirt/enhancements] (not the initial VEP PR)
- [ ] (R) Alpha target version is explicitly mentioned and approved
- [ ] (R) Beta target version is explicitly mentioned and approved
- [ ] (R) GA target version is explicitly mentioned and approved

## Overview

Expose device driver information reported by the QEMU guest agent's `guest-get-devices` command
as a Prometheus metric, giving VM owners a fleet-wide overview of deployed VirtIO
driver versions on Windows VMs. Outdated driver versions can cause performance degradation,
compatibility issues, and may lack security fixes. This metric enables detecting such VMs via
PromQL before problems arise.

KubeVirt also ships a recording rule and an alert flagging VMs whose drivers are not current. Since
"current" differs per device and Windows release, that mapping comes from an optional ConfigMap,
normally shipped by the vendor distributing KubeVirt. Upstream ships none, so nothing is evaluated
and the alert cannot fire on a default installation.

## Motivation

Windows VMs running on KubeVirt use VirtIO drivers for network, storage, balloon, and other
paravirtualized devices. These drivers require periodic updates for performance, bug fixes, and
security patches. Currently there is no centralized way to determine which driver versions are
installed across a cluster - VM owners must check each VM individually.

The QEMU guest agent already supports a `guest-get-devices` command (since QEMU 5.2) that returns
driver name, version, date, and PCI device IDs. KubeVirt has existing infrastructure to poll the
guest agent and expose results as Prometheus metrics. This VEP bridges the gap by adding
`guest-get-devices` to the metrics pipeline.

Knowing the installed version is only half the problem: there is no single "latest VirtIO driver",
so what counts as outdated differs per device and Windows release. Curating that mapping is a
product decision for whoever distributes KubeVirt, but the comparison itself is mechanical - so the
data belongs downstream and the rules belong in KubeVirt.

## Goals

- Expose per-device driver information (name, version, device ID) as a Prometheus metric with
  the driver date as the metric value, enabling both inventory queries and time-based alerting
- Document example PromQL queries for fleet inventory, version distribution, and age-based alerting
- Ship a recording rule and an alert that flag VMs running drivers which are not current, driven by
  a mapping in an optional ConfigMap that upstream leaves empty for distributions or cluster
  administrators to populate

## Non Goals

- Automatically updating drivers inside guest VMs
- Defining what constitutes the "latest" or "correct" driver version
- Basing the built-in alert on driver age; age-based queries remain documented for users who want
  them
- Per-namespace or per-VM policy overrides - the mapping is cluster-wide
- Adding device info to VMI status or the Kubernetes API
- Supporting non-PCI device types beyond what `guest-get-devices` reports
- Supporting Linux guests (the command is Windows-only in the QEMU guest agent)

## Definition of Users

- **VM owners** managing Windows VMs with VirtIO drivers who need visibility into driver versions
  for update planning, fleet health monitoring, and compliance
- **Vendors distributing KubeVirt** who ship the mapping of current driver versions as part of
  their product
- **Cluster administrators** who optionally curate and apply their own mapping on a plain
  upstream install
- **Compliance/audit tools** that need to verify driver versions meet organizational policies

## User Stories

- As a VM owner, I want to query Prometheus to see which driver versions are installed on my
  Windows VMs so I can identify VMs that need driver updates.
- As a VM owner, I want to create a Grafana dashboard showing driver version distribution across
  my VM fleet so I can track update rollout progress.
- As a VM owner, I want to write Prometheus alerts that fire when VMs are running drivers older
  than a policy threshold or a specific version known to have bugs, so I can prioritize updates.
- As a vendor distributing KubeVirt, I want to ship the mapping with my product so that my
  customers get the outdated-driver alert out of the box.
- As a cluster administrator, I want to optionally declare my own mapping on a
  plain upstream install, so that KubeVirt alerts me when VMs fall behind.

## Repos

- https://github.com/kubevirt/kubevirt
- https://github.com/kubevirt/monitoring

## Design

### QEMU Guest Agent Command

The `guest-get-devices` command is a Windows-only QEMU guest agent command available since
QEMU 5.2. It returns VirtIO and QEMU PCI device driver information including name, version,
date, and PCI device/vendor IDs. A typical Windows VM reports 6-10 devices.

Example output:
```json
{
  "return": [
    {
      "driver-date": 1721001600000000000,
      "driver-name": "Red Hat VirtIO Ethernet Adapter",
      "driver-version": "100.95.104.26200",
      "id": {
        "device-id": 4161,
        "vendor-id": 6900,
        "type": "pci"
      }
    }
  ]
}
```

The command is Windows-only. On Linux guests it returns `CommandNotFound`, which the existing
agent command infrastructure handles gracefully. No metric is emitted for VMs without device data.

### Metric Definition

**Name:** `kubevirt_vmi_guest_device_driver_date_seconds`

**Type:** Gauge (using `operatormetrics.NewGauge`; labels set via `CollectorResult` per the domainstats pattern)

**Labels:**

| Label | Example | Source |
|-------|---------|--------|
| `node` | `worker-1` | VMI status (standard) |
| `namespace` | `default` | VMI metadata (standard) |
| `name` | `win-vm-1` | VMI metadata (standard) |
| `driver_name` | `Red Hat VirtIO Ethernet Adapter` | `driver-name` |
| `driver_version` | `100.95.104.26200` | `driver-version` |
| `device_id` | `1041` | `id.device-id` (decimal to hex, no 0x prefix) |

**Value:** Driver date as Unix epoch seconds (converted from the nanosecond timestamp).

Using the driver date as the metric value enables `time() - value` arithmetic in PromQL for
age-based alerting (precedent: `kubevirt_vmi_migration_start_time_seconds`).

Labels omitted by design:
- `vendor_id`: always `1af4` or `1b36` (QEMU hard-codes the filter) - low information density
- `device_type`: always `pci` (the only type QEMU supports) - no information

### Cardinality

The `guest-get-devices` command already filters to VirtIO/QEMU vendor IDs only, so cardinality is
inherently bounded to ~6-10 devices per Windows VM. With 1000 VMs this yields ~6000-10000 time
series - comparable to existing per-item metrics like `kubevirt_vmi_filesystem_capacity_bytes`
(one per filesystem per VM) and `kubevirt_vmi_vnic_info` (one per vNIC per VM).

### Data Path

The command is added to both data pipelines. The domainstats scraper path is gated behind the
`GuestDeviceMetrics` feature gate since it introduces a new Prometheus metric. The VMStatsCollector
path requires only the existing `VMStatsCollector` feature gate - no additional gate is needed
because it simply passes through raw guest agent data, consistent with how other guest agent
commands are handled in that pipeline. Driver info is essentially static during a VM's lifetime,
so infrequent polling is sufficient.

**Domainstats scraper path** (Prometheus metric, gated by `GuestDeviceMetrics`):
```
guest-get-devices (QEMU GA)
  --> virt-launcher agent poller: periodically polls, parses JSON,
      stores in AsyncAgentStore
  --> virt-launcher cmd-server: GetDevices() reads from store
  --> virt-handler domainstats scraper: fetches via gRPC cmd-client
  --> device_metrics.go: emits CollectorResult per device
  --> Prometheus scrape
```

**VMStatsCollector path** (gRPC endpoint, gated by `VMStatsCollector`):
```
guest-get-devices (QEMU GA)
  --> virt-launcher: cached in agentDataCaches with per-command TTL
  --> GetVMStats gRPC: returns raw JSON on demand
```

### Guest Agent Command Invocation

The agent poller in virt-launcher will execute `guest-get-devices` directly via the guest agent
socket rather than through libvirt's API. Libvirt does not currently provide an abstraction for
this command (tracked in [RHEL-235731](https://redhat.atlassian.net/browse/RHEL-235731)). Once
libvirt exposes `guest-get-devices` through its API, the implementation should migrate to use
that abstraction instead of invoking the command directly.

### Driver Version Reference ConfigMap

The mapping of current driver versions lives in an optional ConfigMap named
`kubevirt-guest-device-drivers-config` in the KubeVirt install namespace, normally shipped by the
vendor distributing KubeVirt. Upstream does not create it; only an annotated example is shipped in
`docs/observability/examples/`. Loading follows the existing `kubevirt-vm-labels-config` ConfigMap
and its loader.

An explicit schema `version` is required and rejected if unrecognized, so a
later change cannot be misparsed by an older KubeVirt. Several `driverVersion` entries may share a
`(deviceID, guestOSVersionID)` key, meaning any of them counts as current, which lets a
distribution accept multiple versions, e.g. during a rollout of an upgrade.

`deviceID` is normalized through the same helper virt-handler uses to render the guest agent's
decimal `id.device-id` as hex; drift between the two would silently match nothing.

virt-controller also exposes `kubevirt_guest_device_driver_config_invalid`, a gauge set to `1`
while the ConfigMap present in the cluster is being rejected.

### Reference Metric

**Name:** `kubevirt_guest_device_driver_latest_version_info`

**Type:** Gauge, value always `1`, emitted by virt-controller purely from the ConfigMap,
mirroring the ConfigMap-driven `kubevirt_vm_labels`

**Labels:**

| Label | Example | Source |
|-------|---------|--------|
| `device_id` | `1041` | `devices[].deviceID`, normalized |
| `guest_os_version_id` | `2022` | `devices[].current[].guestOSVersionID` |
| `driver_version` | `100.95.104.26200` | `devices[].current[].driverVersion` |
| `device_name` | `VirtIO network device` | `devices[].deviceName` |

Emission is gated behind `GuestDeviceMetrics`. The recording rule and alert ship ungated, since
KubeVirt cannot feature-gate PrometheusRule content, and are inert with the gate off because they
select metrics that do not exist.

### Recording Rule and Alert

`vmi:kubevirt_vmi_guest_device_driver_outdated:info` yields one series per outdated
(VMI, device) pair:

```promql
max by (namespace, name, device_id, driver_name, driver_version, guest_os_version_id) (
  (
      kubevirt_vmi_guest_device_driver_date_seconds
        * on (name, namespace) group_left (guest_os_version_id)
          topk by (name, namespace) (1, kubevirt_vmi_info)
    and on (device_id, guest_os_version_id)
      kubevirt_guest_device_driver_latest_version_info
  )
  unless on (device_id, guest_os_version_id, driver_version)
    kubevirt_guest_device_driver_latest_version_info
)
```

Read as: attach the VM's Windows release to each of its device series, keep only devices the
mapping actually covers, then drop those whose version is listed as current. What remains is
covered-but-not-current.

`topk` is required because virt-controller runs multiple replicas, so `kubevirt_vmi_info` always
has duplicate series per VMI and the join would otherwise abort the whole rule. `and` is required
because `unless` alone would flag every device the mapping does not cover, which on a default
installation is the entire fleet.

The alert is added to the VM workload alerts:

| Field | Value |
|-------|-------|
| Alert | `OutdatedGuestDeviceDrivers` |
| Expr | `vmi:kubevirt_vmi_guest_device_driver_outdated:info{namespace!=''}` |
| For | `1h` |
| `severity` | `warning` |
| `operator_health_impact` | `none` |

It fires per VMI and device: the `summary` annotation states the condition, and the `description`
names the VM, driver, and version. A corresponding runbook is added to `kubevirt/monitoring`.

### Failure Modes

| Situation | Behavior |
|-----------|----------|
| ConfigMap absent | No reference series, alert cannot fire |
| ConfigMap invalid | Last valid configuration retained, `config_invalid` set to `1` |
| Device has no mapping entry | Skipped |
| `guest_os_version_id` empty | Skipped |
| `kubevirt_vmi_info` missing for a VMI | VM skipped (see Alternatives) |

## API Examples

### Metric Output

```
kubevirt_vmi_guest_device_driver_date_seconds{
  node="worker-1", namespace="default", name="win-vm-1",
  driver_name="Red Hat VirtIO Ethernet Adapter",
  driver_version="100.95.104.26200",
  device_id="1041"
} 1721001600
```

### Example PromQL Queries

List all drivers for a specific VM:
```promql
kubevirt_vmi_guest_device_driver_date_seconds{namespace="default", name="win-vm-1"}
```

Find all VMs with a specific device:
```promql
kubevirt_vmi_guest_device_driver_date_seconds{device_id="1041"}
```

Find all VMs running a specific driver version:
```promql
kubevirt_vmi_guest_device_driver_date_seconds{driver_version="100.95.104.26200"}
```

Find VMs with drivers older than 1 year:
```promql
(time() - kubevirt_vmi_guest_device_driver_date_seconds) > (365 * 24 * 3600)
```

Driver version distribution across cluster:
```promql
count by (driver_name, driver_version) (kubevirt_vmi_guest_device_driver_date_seconds)
```

### Reference ConfigMap

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: kubevirt-guest-device-drivers-config
  namespace: kubevirt
data:
  drivers.yaml: |
    version: v1alpha1
    devices:
      - deviceID: "1041"
        deviceName: "VirtIO network device"   # informational only
        current:
          - guestOSVersionID: "2022"
            driverVersion: "100.95.104.26200"
          - guestOSVersionID: "2022"
            driverVersion: "100.95.104.26100"
          - guestOSVersionID: "2019"
            driverVersion: "100.94.104.25200"
```

## Alternatives

### Add Device Info to VMI Status

Device info could be added to `VirtualMachineInstanceStatus` (like `GuestOSInfo` today).
Rejected because it adds API surface and VMI status update overhead. The primary use case
(monitoring/alerting) is better served by Prometheus metrics. VMI status can be reconsidered
in a future VEP if API access is needed.

### Add Labels to Existing `kubevirt_vmi_info`

Driver info could be added as labels on `kubevirt_vmi_info`. Rejected because there are multiple
devices per VM, which would duplicate the entire `kubevirt_vmi_info` series per device and
massively increase cardinality of the central VMI info metric.

### Use Only VMStats REST Endpoint

The metric could be collected only via the VMStats REST endpoint (behind `VMStatsCollector`
feature gate) instead of the domainstats scraper. Viable future option when VMStatsCollector
matures to include its own Prometheus metrics layer. For alpha, the command is added to both
pipelines - the VMStatsCollector path passes through raw data under its own feature gate, while
the Prometheus metric comes from the domainstats scraper under `GuestDeviceMetrics`.

### Info-style Metric with Value = 1

Instead of encoding the driver date as the metric value, the metric could follow the standard
`_info` convention with value always set to 1 and the driver date as a label
(`kubevirt_vmi_guest_device_info{..., driver_date="1721001600"} 1`). More consistent with other
KubeVirt info metrics, but loses `time() - value` arithmetic for the user-written age queries
documented above.

### Age-Based Alert

An alert could fire on `time() - kubevirt_vmi_guest_device_driver_date_seconds` exceeding a
threshold. Rejected because a stable device's current driver can be more than a year old, so
any age threshold might produce false positives.

### Compare Driver Dates Instead of Versions

The mapping could carry the release date of the current driver and flag anything strictly older,
sidestepping version-string ordering. Rejected because it requires a date to be maintained per
entry, and because an arithmetic comparison demands unique matching on both sides where the set
operators used here tolerate duplicates for free.

### Generate the PrometheusRule from the ConfigMap

A controller could rewrite the PrometheusRule expression from the ConfigMap instead of exposing a
metric. Rejected: it makes rule content dynamic and untestable by upstream CI, adds a second
writer to an object virt-operator already reconciles, and reloads the rule group on every edit.

### Add `guest_os_version_id` to the Device Metric

The Windows release could be a label on `kubevirt_vmi_guest_device_driver_date_seconds` rather
than joined from `kubevirt_vmi_info`. That would remove the join, the `topk` guard, and the last
row of the Failure Modes table, since a VM would no longer depend on a second component's metric to
be evaluated. Rejected to avoid duplicating what `kubevirt_vmi_info` already carries.

## Scalability

- ~6-10 time series per Windows VM; inherently bounded by QEMU's vendor ID filter
- Infrequent polling interval minimizes guest agent load
- Driver info is static during a VM's lifetime, so stale reads are not a concern
- The reference metric is one series per mapping entry - tens of series in practice
- The recording rule is a filter, so its output is bounded by the number of *outdated* devices, near
  zero in a healthy fleet, and `max by` strips `node` and all user-defined VM labels so it cannot
  inherit unbounded cardinality

## Update/Rollback Compatibility

- The Prometheus metric path and the reference metric are guarded by the `GuestDeviceMetrics`
  feature gate; the VMStatsCollector path is guarded by the existing `VMStatsCollector` feature gate
- No changes to VMI spec or status - the metric is collected and exposed independently
- No impact on VM runtime behavior
- virt-operator neither creates nor reconciles the ConfigMap, so whoever ships it keeps ownership
  across upgrades.
- On rollback the rules disappear; an orphaned ConfigMap is harmless.

## Functional Testing Approach

- Unit tests for JSON parsing of `guest-get-devices` response
- Unit tests for metric emission with mock device data (following the pattern in
  `filesystem_metrics_test.go`)
- Unit tests for the ConfigMap loader, following the pattern in `labels_config_test.go`
- promtool tests in `hack/prom-rule-ci/prom-rules-tests.yaml` cover the whole rule and alert path
  over synthetic series, needing no Windows guest and no cluster.
- A Linux-only e2e test in `tests/monitoring/` applies, mutates, and deletes the ConfigMap and
  asserts the reference metric follows
- e2e testing with a real Windows VM is not feasible in kubevirt/kubevirt CI due to Windows
  licensing constraints. The data path reuses existing guest agent infrastructure already covered
  by e2e tests for other commands. Full e2e validation can be done downstream by vendors with
  access to Windows images.

## Implementation History

## Graduation Requirements

### Alpha

- [ ] Feature gate `GuestDeviceMetrics` guards the domainstats scraper / Prometheus metric path
- [ ] `guest-get-devices` command added to agent poller and VMStatsCollector pipelines
- [ ] Metric emitted per device per VM
- [ ] Unit tests for JSON parsing and metric emission
- [ ] ConfigMap loader, with `deviceID` normalization shared with the emitter
- [ ] Reference metric and `config_invalid` gauge, gated by `GuestDeviceMetrics`
- [ ] The recording rule and the `OutdatedGuestDeviceDrivers` alert, with promtool tests
- [ ] Runbook merged in kubevirt/monitoring
- [ ] Example ConfigMap in `docs/observability/examples/`
- [ ] Initial documentation

### Beta

- [ ] Gather user feedback on label selection and metric structure
- [ ] Review cardinality impact from real-world deployments
- [ ] Refine polling interval if needed based on operational experience
- [ ] Gather feedback on the ConfigMap schema, including whether `guest_os_version_id` is granular
      enough or a `guest_os_kernel_release` build dimension is needed
- [ ] Reconsider labeling the device metric with `guest_os_version_id` directly if the
      `kubevirt_vmi_info` join proves fragile
- [ ] Documentation finalized

#### On-By-Default Readiness

- [ ] With the feature gate on and no ConfigMap present, the alert provably cannot fire
- [ ] No measurable cardinality impact from the metric in scale testing
- [ ] Guest agent polling for `guest-get-devices` shown not to affect guest performance at scale

### GA

- [ ] Feature gate promoted to GA
- [ ] ConfigMap schema promoted to `version: v1`, with documented `v1alpha1` compatibility
- [ ] Migrate to libvirt's `guest-get-devices` abstraction once available
  ([RHEL-235731](https://redhat.atlassian.net/browse/RHEL-235731))
