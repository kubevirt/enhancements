...
The design will ensure that the current method for container disks and kernel boot continues to work by default. 
The new method using imageVolumes will only be used once the feature gate is enabled. By default, the feature 
gate will be off until it is fully graduated.
...
this approach may compromise the isolation of the compute container where the domain runs, as discussed 
...
- Add the new implementation, protected by a feature gate that is disabled by default.
...
- During the upgrade to a version where the feature gate is graduated, ensure all launcher pods are updated through the live migration process.
...
- Add support for kubevirtci to enable Kubernetes feature gates when setting up the cluster.
- Add a periodic test lane to run all relevant tests with the new approach.

### GA
- Wait for the imageVolume feature gate to be graduated to GA in Kubernetes.
- Ensure periodic tests pass, confirming the new method works as expected.
