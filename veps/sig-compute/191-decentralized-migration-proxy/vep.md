...
- This VEP targets alpha for version: 1.9
- This VEP targets beta for version: 1.10
- This VEP targets GA for version: 1.11
...
### Alpha
It will be hidden behind a feature gate called `CrossClusterMigrationProxy`. If the feature gate is not enabled the `crossClusterLiveMigration` network field will be rejected by the webhook. This will allow users to opt-in or out at will without affecting any decentralized live migrations.

### Beta
make feature gate enabled by default per beta requirements, as it is an extension of decentralized live migration

### GA
remove feature gate, the proxy will be enabled automatically if the user sets a `crossClusterNetwork`
