...
**0.5** (halve the patience each round) felt **reasonable**: patience for successive relaxation targets decays **fast enough** that the job does not sit too long on a `bestRemainingBytes` level that the workload or network may never hit again, while still giving each level a meaningful window anchored initially to `stallProgressTimeout`. Notice that there is **no minimum floor** on patience and this is intentional.
...
`completionTimeoutFactor` is a scaling factor used in the following budget formula:

`impliedDowntime <= (maxCompletionTime × completionTimeoutFactor) - elapsedTime`

See **Design §Switch-over Budget Check** in the parent VEP. This factor is the remaining-budget cutoff: start a switch-over to post-copy / stop-and-copy only if implied downtime still fits; otherwise abort.

KubeVirt today hard-codes a ×2 second phase after the 1× force-converge. Default **2.0** matches that abort semantics.