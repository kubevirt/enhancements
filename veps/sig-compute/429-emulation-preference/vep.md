...
A new field `crossArchEmulation` is introducev to kubevirt and VM/VMI Spec.
...
1. **Per-VM** — `vmi.Spec.crossArchEmulation` (if non-empty)
2. **Global** — `clusterConfig.GetcrossArchEmulation()` (if non-empty)
...
spec:
  architecture: arm64
  crossArchEmulation: Software # Allows software emulation, overrides cluster policy
  running: true
  template:
    spec:
      domain:
