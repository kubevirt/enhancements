...
type LayerAnnotationSelector struct {
    ...
    // (e.g. "io.kubevirt.disk.name").
    Key string `json:"key"`
    // Value is the expected annotation value (e.g. "rootdisk").
...
(the `default` overlay), which issues the metrics serving certificate. A
second `virt-operator` overlay is provided for Phase 3 integration.
...
* Two Kustomize overlays: `default` (cert-manager), `virt-operator`
  (metrics serving certificate managed by `virt-operator`)
...
index or manifest, resolves the platform, fetches the config blob,
validates the artifact structure, and writes the artifactType, config
blob, layer info (disk name, size), and resolved architecture in a
structured JSON format to stdout. All other log messages are emitted to
stderr. After the Job succeeds, the controller reads stdout from the
Job's Pod logs and uses the parsed output to create DataVolumes and the
target VM or VMTemplate. This avoids granting the controller
...
Import Controller (virt-import)
  ...
  |       d. Fetches config blob
  |       e. Validates artifact:
  ...
  |            - No duplicate io.kubevirt.disk.name values across layers
  |            - io.kubevirt.disk.size must be a valid Kubernetes
  |              resource quantity
  |            - Every PVC volume in the config blob must have a
  |              matching disk layer (by io.kubevirt.disk.name), and
  |              every disk layer must have a matching PVC volume
  |            Non-compliant artifacts cause the Job to fail.
  |       f. Writes JSON to stdout: artifactType, config blob, layer
  |          info (disk name, size), resolved architecture
  ...
  |     |     source.registry.platform.architecture = <resolved-arch>
  |     |     source.registry.layer.annotation.key = io.kubevirt.disk.name
  |     |     source.registry.layer.annotation.value = <disk-name>
  ...
  |     v
  |     CDI:
  |       Resolves manifest (if image index, select by Platform;
  |         if plain manifest, verify Platform matches)
  |       Finds layer matching annotation key-value
  |       Fetches blob via GetBlob() using matched layer's digest
...
`kubevirt/virt-import`. `virt-operator` issues and rotates the controller's
metrics serving certificate, following the same pattern as
`kubevirt/virt-template`.
...
* The CDI `Layer` field is additive. On rollback to a CDI version
  without it, DataVolumes with `layer` set would fail CDI validation,
  causing the DataVolume to fail. Since import uses all-or-nothing
  semantics, the entire import fails in this case.
