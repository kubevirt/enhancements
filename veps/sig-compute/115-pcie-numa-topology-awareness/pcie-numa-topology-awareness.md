...
- Support VM creation with NUMA topologies that map directly to the underlying
  host topology.

## Non Goals

- This VEP focuses solely on ensuring optimal guest PCIe topology generation
  when devices are locally available. It does not address changes to Kubernetes
  device/resource allocation for guaranteeing NUMA-aligned device allocation.
  Dynamic Resource Allocation (DRA) will help address that in the near future.
...
This feature will be introduced behind a feature gate named `PCINUMAAwareTopology`, as this
will allow the community to test, validate and provide feedback before enabling it by default.
...
### Alpha
- Guest VM NUMA topology awareness for host devices implemented behind the respective feature gate
