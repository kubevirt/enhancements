...
- **Conflict retries**: Two controllers updating the same resource race
  on `resourceVersion`. In practice, controllers resolve this in one of
  two ways: (1) let the reconciler retry — straightforward but generates
  unnecessary API server load that compounds under contention; or (2)
  the controller detects the conflict and issues an explicit `Get` call
  to refresh state before retrying — more targeted but still adds an
  extra round-trip per conflict, and does not scale as informer-based
  cached reads would.
...
**Not recommended for initial adoption**: Core VMI phase and status
reconciliation in `pkg/virt-handler/` and
`pkg/virt-controller/watch/vmi/`. This exclusion is specifically about
the shared phase/status reconciliation path: virt-controller writes VMI
status via JSON Patch with test-and-set, while virt-handler writes via
full Update, with conditional logic that depends on the current VMI
phase. The multi-writer coordination across virt-controller,
virt-handler, and migration pods (source + target) makes this path
unsuitable for early SSA adoption.

Isolated, non-conflicting fields within `virt-controller`'s VMI
sub-controllers — such as finalizer management or specific annotation
updates where a single actor has clear ownership — may be evaluated for
SSA adoption after the pattern is proven on simpler controllers.
Whether any such fields are good candidates will be determined
per-field during the incremental review process.
...
|------|-------------|------------|
| **List merge data loss** | If a CRD condition slice lacks `x-kubernetes-list-type: map` / `x-kubernetes-list-map-keys: [type]` annotations, an Apply that sends a single condition takes ownership of the entire list and removes all other entries. | Audit and annotate all condition slices before any controller migration that targets list fields. Tracked as an explicit Alpha graduation requirement. Each adoption PR reviewer must confirm CRD schema is correct. |
| **Informer cache bloat** | SSA increases the size of `managedFields` per object. With many field managers, cached objects carry significantly more metadata, increasing memory footprint of informers. | Strip `managedFields` from informer caches using a transform function (see Scalability section). Evaluate per informer cache in each adoption PR. |
| **Transient ownership flips** | During a rolling DaemonSet update (e.g. virt-handler), old pods use `Update` and new pods use `Apply` on the same resource simultaneously. The old pod's `Update` may temporarily re-take ownership of fields the new pod applied. | Use `Force: true` on all Apply calls so new-version pods always succeed. The transient flip lasts only for the rollout window. Each migration PR must evaluate whether transient ownership reversal causes incorrect behavior. |
...
This also applies to external actors: when any client — internal or
external to KubeVirt — performs an `Update` without `managedFields` in
the object metadata, the Kubernetes API server resets field ownership
to that updating manager for all fields included in the update. This
is standard, documented SSA behavior and does not corrupt data, but it
can cause SSA field ownership to be silently cleared. Users who have
external components that strip `managedFields` from their informer
caches and then write back via `Update` (for example to set labels or
annotations) should be aware that this will reset ownership tracking
for every field in the update payload. The correct approach for such
external actors is to use SSA themselves (with a distinct field manager
for the fields they own) rather than `Update` on a stripped object.
...
- [ ] All condition slices in KubeVirt CRDs (`kubevirt.io/api`) audited
      and annotated with `// +listType=map` and `// +listMapKey=type`
      (prerequisite before any controller migration targeting list fields)
...
- [ ] All initial candidate controllers from the candidate list
      (ReplicaSet, Pool, Snapshot, Export, Clone, Operator) have
      completed migration to SSA
- [ ] Any remaining `Update`/read-modify-write call sites across core
      controllers have a documented rationale in the adoption PR (or in
      a tracking issue) for why SSA was not adopted for that call site
- [ ] Stable across at least two KubeVirt releases with no SSA-related
