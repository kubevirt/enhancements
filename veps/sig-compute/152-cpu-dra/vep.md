...
That driver is an out-of-tree alternative to kubelet CPU Manager: it discovers node CPU topology, publishes CPUs via `ResourceSlice` (individual per-CPU devices or grouped consumable capacity), and isolates claimed workloads by pinning the consumer container cgroup through NRI.
...
Operators can tune Topology Manager to improve CPU/device alignment, but that remains kubelet-local after bind. Exclusive CPUs can miss the NUMA of a GPU or SR-IOV NIC, or the pod can fail after the scheduler has already placed it.
...
- Managed claim (see [Appendix: Phased follow-ups](#appendix-phased-follow-ups); covered by VEP #300)
...
- As a user, I want exclusive DRA CPUs by setting `dedicatedCpuPlacement` the way I do today but with a dra-driver, without writing a `ResourceClaim`
...
- **virt-controller** does not synthesize a CPU `ResourceClaim` and uses the existing dedicated-CPU / kubelet CPU Manager pod path — the same path as when the feature gate is off.

#### After CPU and memory DRA drivers

Users still set `dedicatedCpuPlacement` and `guestMappingPassthrough`. Once a memory DRA driver exists, virt-controller or VEP #300 can align CPU and memory claims with `matchAttribute` on NUMA so the scheduler places both on the same node or leaves the pod `Pending`. Topology Manager `best-effort` does not guarantee that today, and CPU DRA alone gives Memory Manager no CPU hint — that is why Alpha refuses the combination. Multi-cell passthrough stays [VEP #115](../115-pcie-numa-topology-awareness/pcie-numa-topology-awareness.md); this Alpha stays XOR with `guestMappingPassthrough`.
...
- **KubeVirt owns:**
  ...
  - applying the allocated exclusive CPUs to the libvirt domain XML via the existing dedicated-CPU cgroup cpuset path
- **User owns:**
...
- Driver capability requirements:
  ...
  - Must pin the virt-launcher compute container's cgroup to the exclusive CPUs allocated to the synthesized claim (NRI), the same isolation kubelet CPU Manager provides today
...
- When `CPUsWithDRA` is on and `dedicatedCpuPlacement` is set, reject `numa.guestMappingPassthrough` at admission. See [Mutual exclusivity with guest NUMA passthrough](#mutual-exclusivity-with-guest-numa-passthrough).
...
- Attaches the claim to the virt-launcher pod via `spec.resourceClaims[]` (`resourceClaimName`) and `containers[].resources.claims[]`, merging with any user claims from `vmi.spec.resourceClaims` (GPUs, NICs, host devices from VEP #10). Alpha does **not** treat a user-authored CPU claim in `vmi.spec.resourceClaims` as the exclusive-CPU path.
...
- Uses the existing dedicated-CPU path: read the compute container's cgroup cpuset and write libvirt `<cputune><vcpupin .../></cputune>`. Emulator and supplemental IO thread pins reuse that same path when those features are enabled.
- Does **not** parse `DRA_CPUSET_*` or other per-claim CPU env vars (see [Per-claim DRA_CPUSET environment variables](#per-claim-dra_cpuset-environment-variables)).
- The `dra-driver-cpu` NRI plugin pins the virt-launcher compute container's cgroup to the exclusive CPUs allocated to the synthesized claim. virt-launcher then sees the same isolated cpuset it sees today under kubelet CPU Manager.
...
After the driver prepares the claim, NRI pins the compute container cgroup to the allocated exclusive CPUs (for example `0-9`). virt-launcher writes `<vcpupin>` from that cgroup cpuset, the same as today's `dedicatedCpuPlacement` path.
...
`vmi.spec.resourceClaims` is the VEP #10 field for user-authored GPU/NIC/host-device claims. It is **not** an Alpha escape hatch for an already-authored CPU claim: exclusive CPUs come from `dedicatedCpuPlacement` and the synthesized claim. A later follow-up can wire a user-authored CPU claim there if needed; regular exclusive-CPU users never write it.

### Per-claim DRA_CPUSET environment variables

`dra-driver-cpu` inject `DRA_CPUSET_<claimUID>=<cpuset>` via CDI and have virt-launcher parse those env vars to write `<vcpupin>`.

Rejected. One exclusive synthesized claim looks unambiguous, but the virt-launcher pod can carry **multiple** CPU claims. A dedicated claim is the set virt-launcher must pin 1:1; a shared claim is a floating set. Each claim gets its own `DRA_CPUSET_<claimUID>`, and nothing in the env name says which is dedicated and which is shared. virt-launcher cannot choose the right pin set vs float set from env vars alone.

The same ambiguity shows up for mixed exclusive/shared guests in [Phase 3](#phase-3--advanced-use-cases-host-allocation-intent): domain XML needs host-allocation intent on the VMI, not a bag of per-claim cpusets.

Alpha therefore keeps the existing `dedicatedCpuPlacement` path: NRI isolates the compute container cgroup to the exclusive allocation, and virt-launcher pins from that cgroup. No new env-var parser in virt-launcher. In future, we can extend it to the env variable as a performance improvement in the future.
...
See [VEP #10](../10-dra-devices/vep.md#scalability) for details. virt-launcher does not watch `ResourceClaim` or `ResourceSlice`. It pins from the compute container's cgroup cpuset (the existing dedicated-CPU path).
...
virt-controller synthesizes the `ResourceClaim`; virt-launcher pins from the container cgroup. A rolling upgrade can run those at different versions. The gate is off by default, so clusters that have not enabled it are unaffected.

Both old and new virt-launcher already use the dedicated-CPU cgroup path, so mixed versions do not change guest pinning.

| Controller | Launcher | Claim | Guest pinning |
| --- | --- | --- | --- |
| new | new | yes | cgroup cpuset (NRI isolated by DRA) |
| new | old | yes | cgroup cpuset (NRI isolated by DRA) |
| old | new | no | cgroup cpuset (CPU Manager if `dedicatedCpuPlacement`) |
| old | old | no | CPU Manager / existing `dedicatedCpuPlacement` |

**New controller / old launcher:** NRI still pins the container to the claimed exclusive CPUs. The old launcher already writes `<vcpupin>` from that cgroup.

**Old controller / new launcher:** no CPU claim is synthesized. The new launcher uses the same cgroup path under CPU Manager.

## Functional Testing Approach

- **Unit tests (Alpha):** all KubeVirt-owned behavior, including the negatives the design already implies. `hostCPUs` accounting; synthesized grouped claim (DeviceClass `dra.cpu`); claim attach/merge with user GPU/NIC claims; virt-api reject of `numa.guestMappingPassthrough` when the gate is on; virt-launcher continues to pin from the compute container cgroup cpuset; missing `DeviceClass` (or claim/pod cannot be rendered) → virt-controller does not start the virt-launcher pod.
- **No mock e2e.** Allocation and NRI cgroup isolation require a DRA CPU driver. Without one the virt-launcher stays Pending on an unallocated claim. Claim rendering and the negatives above are unit-tested instead.
- **Manual / external (Alpha):** validation with [`dra-driver-cpu`](https://github.com/kubernetes-sigs/dra-driver-cpu). Not an in-tree CI-gating lane.
...
- virt-launcher pins from the compute container cgroup cpuset (existing dedicated-CPU path); NRI isolates that cgroup to the synthesized claim
...
- Feature gate **on by default** (still disable-able)
...
virt-controller turns that into one grouped exclusive claim (here `4 + 1 + 2` host CPUs). NRI isolates the compute container cgroup to that set; virt-launcher pins from the cgroup. Every domain vCPU is exclusive and 1:1 with a host CPU in the allocated set.
