...
Replace KubeVirt's hardcoded OVMF firmware paths for EFI Secure Boot with
libvirt's firmware auto-selection feature. Instead of specifying explicit
`<loader>` and `<nvram>` paths in the domain XML, KubeVirt will use
`<os firmware='efi'>` with `<firmware><feature>` elements, delegating
firmware selection to libvirt based on JSON descriptor files shipped by the
`edk2-ovmf` package.
...
1. Set `<os firmware='efi'>` with `<firmware><feature>` elements requesting
   `secure-boot` and `enrolled-keys`.
2. Let libvirt match these requirements against its firmware descriptor files.
3. Let libvirt resolve the loader path, NVRAM template, and NVRAM file path.
...
This is the primary area requiring careful consideration. Currently KubeVirt
explicitly sets the NVRAM path via `PathForNVram()`:

- Root VMs: `/var/lib/libvirt/qemu/nvram/<vmi>_VARS.fd`
- Non-root VMs: `/var/run/kubevirt-private/libvirt/qemu/nvram/<vmi>_VARS.fd`

With firmware auto-selection, libvirt determines the NVRAM path based on its
`nvram_dir` configuration in `qemu.conf`. KubeVirt's embedded libvirt uses:

- Root: The system default (`/var/lib/libvirt/qemu/nvram/`)
- Non-root: A copy of the system `qemu.conf` at
  `/var/run/kubevirt-private/libvirt/qemu.conf`

For root VMs, the paths match. For non-root VMs, verification is needed to
ensure libvirt's auto-selected NVRAM path aligns with KubeVirt's expectations
for the volume mount at `PathForNVram()`. If they diverge, the `qemu.conf`
copy must be updated to set `nvram_dir` explicitly.
...
The gate is checked in `manager.go` when constructing the
`EFIConfiguration`. If the gate is disabled or the VM uses confidential
...
if secureBoot && vmType == efi.None && config.FirmwareAutoSelectionEnabled() {
    ...
    // or when the feature gate is disabled.
}
```
...
2. **`pkg/virt-launcher/virtwrap/api/schema.go`**: Add `Firmware` attribute
   and `FirmwareInfo`/`FirmwareFeature` types to the `OS` struct.

3. **`pkg/virt-launcher/virtwrap/converter/types/converter-context.go`** and
   **`pkg/virt-launcher/virtwrap/converter/compute/os.go`**: Add
   `UsesFirmwareAutoSelection` field to `EFIConfiguration`.

4. **`pkg/virt-launcher/virtwrap/converter/compute/os.go`**: When
   `UsesFirmwareAutoSelection` is true, set firmware auto-selection XML
   instead of explicit loader/NVRAM.

5. **`pkg/virt-launcher/virtwrap/manager.go`**: Check
