...
Non-Secure-Boot EFI is excluded from the alpha scope to limit the initial
change surface, but there is no fundamental technical blocker preventing
firmware auto-selection for this case.

As clarified by the libvirt maintainer in the VEP review, requesting
`<feature enabled='no' name='enrolled-keys'/>` correctly matches both
firmware descriptors with no Secure Boot support compiled in (e.g.
`51-edk2-ovmf-2m-raw-x64-nosb.json`) and those where Secure Boot is present
but the NVRAM template doesn't have keys enrolled (e.g.
`41-edk2-ovmf-2m-raw-x64-sb.json`). The descriptor sorting is intentional,
so the SEV-capable descriptor (`50-edk2-ovmf-x64-nosb.json`) does not
introduce ambiguity — descriptors for "regular" builds are sorted before
those intended for the CVM use case.

This means non-Secure-Boot EFI could be added to firmware auto-selection
in a subsequent release cycle by requesting `enrolled-keys=no`, following
the same pattern used by
[virt-install](https://github.com/virt-manager/virt-manager/pull/1038).
...
(since `OVMF_CODE.fd` / `OVMF_VARS.fd` are stable names), so deferring this
carries no risk.
...
The table below reflects the **alpha** scope. Non-Secure-Boot EFI is a
candidate for expansion in a subsequent release (see
[Non-Secure-Boot EFI](#non-secure-boot-efi) above). SEV/SEV-ES remain
blocked on missing firmware descriptors (see [Remaining Blockers](#remaining-blockers)).
...
| EFI + TDX | Explicit paths (stateless) | Explicit paths (unchanged) | Explicit paths (unchanged) |
...
For **new VMs**, qcow2 NVRAM could offer reduced memory usage and
copy-on-write benefits. However, for alpha we use `format='raw'` uniformly
to keep the upgrade/rollback path simple — a VM created with the feature
gate enabled and then rolled back to a version without it must still be
able to use the same NVRAM file with explicit pflash paths. Supporting
qcow2 NVRAM for new VMs could be explored in beta or GA once the
`uefi-vars` path (see [Future Work](#future-work)) eliminates pflash NVRAM
entirely.
