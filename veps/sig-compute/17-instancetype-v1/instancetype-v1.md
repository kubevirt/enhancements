...
`v1` through an intermediate `v1beta2` version, introducing three key changes:
...
3. **Resource visibility in VM status**: vCPU and memory resources applied from
   a referenced instancetype are surfaced under
   `vm.status.instancetypeRef.resources`. User-overridden fields are omitted,
   so the status reflects only what the instancetype actually contributed.
...
- Surface vCPU and memory resources actually applied from the instancetype in
  `vm.status.instancetypeRef.resources`, omitting user-overridden fields
...
- As a VM user, I want to see which resources the instancetype contributed in
  the VM status, without fetching the instancetype object or calling
  expand-spec.
...
### Part 3: Resource Visibility in VM Status

An `InstancetypeStatusResources` struct is added to `InstancetypeStatusRef`,
surfacing vCPU topology and memory applied from the instancetype under
`vm.status.instancetypeRef.resources`.

#### API Types

Both fields are optional pointers, populated only when the instancetype
contributes that resource (not overridden by the VM spec):
...
Added to `InstancetypeStatusRef`:

```go
type InstancetypeStatusRef struct {
    // ...existing fields...

    // Resources shows which CPU and memory resources were applied from the
    // referenced instancetype. Fields overridden by the user are omitted.
    //
    // +optional
...
The VM controller populates `status.instancetypeRef.resources` after applying
the instancetype. The CPU topology reflects the final computed layout after
applying the preferred topology from `VirtualMachinePreference` if present, or
the default topology (sockets) otherwise. If the user overrides CPU or memory
via custom sizing, the corresponding field is omitted.

**No overrides** - both CPU and memory from instancetype:
...
**User overrides memory** - only CPU shown:
...
For device-only instancetypes or when the user overrides everything, the
`resources` field is not populated.
...
- Create a VM referencing an instancetype without overrides. Verify
  `vm.status.instancetypeRef.resources` contains both CPU and memory.
- Create a VM with memory overridden. Verify `resources` contains only CPU.
- Change the instancetype reference. Verify `resources` updates accordingly.
...
- [ ] `InstancetypeStatusResources` added to `InstancetypeStatusRef`; VM
  controller populates `status.instancetypeRef.resources` with only resources
  applied from the instancetype (user-overridden fields omitted)
