...
1. **Announce the policy** on `kubevirt-dev@googlegroups.com` and in the
   release notes for the next GA release after this VEP is accepted.
2. **Grace period**: Provide one full release cycle of notice. For example, if
   announced with v1.8 GA, the policy takes effect at v1.9 GA.
3. **Initial EOL batch**: At the policy effective date, all releases older than
   ...
   v1.9 GA, this would mean v1.6 and earlier are EOL.
