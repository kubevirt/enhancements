...
- As a reviewer of an implementation PR in kubevirt/kubevirt, I want CodeRabbit to verify that
  the change aligns with the design described in the referenced VEP in kubevirt/enhancements —
  for example, flagging when an implementation deviates from the API shape or phasing plan
  described in the VEP — because VEP compliance today relies entirely on the reviewer remembering
  to check the linked VEP manually.
...
- [kubevirt/hyperconverged-cluster-operator](https://github.com/kubevirt/hyperconverged-cluster-operator) — initial adopter
- [kubevirt/monitoring](https://github.com/kubevirt/monitoring) — initial adopter
...
Project-specific context is kept in documentation files — `CONTRIBUTING.md`, architecture
documents, VEP templates — that are maintained for human readers, independently of any tooling.
CodeRabbit reads these files as-is; it does not require a separate, tool-specific knowledge
representation. `.coderabbit.yaml` controls which CodeRabbit features are active and how the
tool navigates the codebase, but the knowledge itself lives outside the configuration file.

This separation means the same documentation serves all consumers: human contributors,
reviewers, and AI agents alike. Improving project documentation benefits the whole ecosystem,
not just CodeRabbit.

The initial set of documents per repository:
...
`knowledge_base.linked_repositories` gives CodeRabbit cross-repo context during reviews,
enabling it to trace dependencies, verify API compatibility, and surface breaking changes across
related codebases. Each repository lists the repos it depends on or that depend on it.

For example, kubevirt/kubevirt and kubevirt/enhancements have a natural dependency: implementation
PRs in kubevirt/kubevirt reference VEPs in kubevirt/enhancements. Linking them lets CodeRabbit
verify that an implementation aligns with the API shape, phasing plan, and non-goals described
in the referenced VEP, without the reviewer having to locate and cross-check it manually.

```yaml
# Example: kubevirt/kubevirt linking to kubevirt/enhancements
knowledge_base:
  linked_repositories:
    - repository: "kubevirt/enhancements"
      instructions: "Contains VEP design documents. When a PR references a VEP number, locate
        the corresponding vep.md and verify the implementation aligns with the stated API shape,
        phasing plan, and non-goals."
```

Each repository's maintainers determine which linked repositories are relevant and provide
instructions that describe the nature of the dependency.
...
| **Vendor dependence** | Project-specific context lives in documentation files maintained independently of CodeRabbit. These files remain useful regardless of which review tool reads them. `.coderabbit.yaml` is the only file specific to CodeRabbit; removing the GitHub App or switching to another tool leaves the documentation intact. Adoption is fully reversible. |
...
| **Bot/automation PR noise** | Configure `reviews.auto_review.ignore_title_keywords` or author-based filtering to skip PRs from known automation accounts (renovate, dependabot, kubevirt-bot). Without this, CodeRabbit reviews routine dependency bumps and automated PRs that need no AI feedback. |
