...
3. **Error Handling**: If the value is invalid, the webhook rejects the
   deletion request with a descriptive error.
4. **Error Handling**: If the patch fails, the webhook rejects the
   deletion request with a descriptive error.
5. **Deletion Propagation**: The VM deletion proceeds as usual. The updated
