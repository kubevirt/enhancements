...
2. **Visibility and Tracking**: The Author of VEP will open an issue to track their progress, maturity stages (alpha,
...
3. **SIG Review and Collaboration**: Each VEP will have one target SIG. In case of a cross SIG feature, the most relevant 
   SIG should be chosen. The SIG will decide if the VEP is accepted and whether it should be tracked for the upcoming release. 
...
4. **Centralized Prioritization**: At the start of each release cycle, all accepted VEPs will be designated as the
   ...
   support and a commitment to implementation.
...
In addition, they are encouraged:

- talk about the design in the unconference sessions to bring everyone on the same page about the problem, 
  use-cases/design etc...
- join the relevant sig calls for further discussions, and seeking for reviews.
...
1. Re-targeting of VEP - In case of implementation not converging, new blockers being discovered, pushback of community
   or withdrawal of an approver the VEP needs to be re-targeted. The VEP needs to be updated with a new target. The SIGs
   should shift the focus on tracked VEPs.
...
Acceptance of an enhancement doesn't guarantee that the feature will land in the current or later release. The process is
collaborative effort between contributors and approvers. Features not landing in the release branch prior to CF will need
to file for an exception, see [Exceptions](#Exceptions)
...
Exceptions are serving for any edge case that is not specified in this document, by release team/repository or within
KubeVirt repository.
Typically, an exception would be asked to allow contributors to continue to working on VEP/PRs/code after the EF or CF
respectively.
Exceptions can be asked before actual EF/CF.
